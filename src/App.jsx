...
export default function App(){
  ...
  async function enterApp(mode){const next=normalizeProfile({...profile,name:profile.name||makeAnonName()});setProfile(next);localStorage.setItem('qc_profile',JSON.stringify(next));if(!isSupabaseConfigured||mode==='guest'){localStorage.setItem('qc_entered',JSON.stringify(true));setEntered(true);setAuthMode(mode);return;}try{if(mode==='google'){localStorage.setItem('qc_profile',JSON.stringify(next));await supabase.auth.signInWithOAuth({provider:'google',options:{redirectTo:window.location.origin}});return;}if(!email.trim()){setAuthNotice('Enter your email first.');return;}const{error}=await supabase.auth.signInWithOtp({email:email.trim(),options:{emailRedirectTo:window.location.origin}});if(error)throw error;await syncProfile(next,null,'email_magic_link');setAuthNotice('Magic link sent. Check your email. For beta preview, you can continue as guest.');}catch(err){setAuthNotice(err.message||'Auth setup needs provider configuration. Continuing in beta mode.');}}
