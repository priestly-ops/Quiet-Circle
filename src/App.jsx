...
export default function App() {
  ...
  function sendCompanionMessage() {
    ...
    const botMsg = {
      from: 'Quiet Circle',
      text: 'Thank you for trusting this space. Try naming the feeling, taking one slow breath, and choosing one tiny next step. If you are in danger, please contact emergency support now.'
    };
  ...
  function sendRoomMessage() {
    ...
    const next = [...roomMessages, { user: profile.name || 'Anonymous', text: roomInput.trim() }];
    setRoomMessages(next);
    localStorage.setItem('qc_room_messages', JSON.stringify(next));
    setRoomInput('');
  }

  function reportContent(type, text) {
    const next = [{ type, text, status: 'pending', at: new Date().toLocaleString() }, ...reports];
    setReports(next);
  ...
  const nav = [
    ['dashboard', 'Dashboard'], ['profile', 'Profile'], ['mood', 'Mood'], ['journal', 'Journal'],
    ['companion', 'AI Companion'], ['rooms', 'Rooms'], ['crisis', 'Crisis Help'], ['admin', 'Admin']
  ];
    ...
    <div className="appShell">
      <aside className="sidebar">
        <div className="brand">
          ...
          <div><h1>Quiet Circle</h1><p>Private mental wellness MVP</p></div>
        </div>
        <nav>
          {nav.map(([id, label]) => <button key={id} className={active === id ? 'nav active' : 'nav'} onClick={() => setActive(id)}>{label}</button>)}
        </nav>
        <div className="safetyBox">Not medical care. If you are in immediate danger, call emergency services.</div>
      </aside>

      <main className="main">
        {active === 'dashboard' && <section className="page">
          <div className="heroPanel"><p className="eyebrow">Welcome back, {profile.name}</p><h2>A calmer place to pause, reflect, and feel heard.</h2><p>Track your mood, journal privately, talk with a supportive companion, and enter anonymous support circles.</p></div>
          <div className="statsGrid">
            ...
            <div className="stat"><span>{reports.length}</span><p>Reports pending</p></div>
            <div className="stat"><span>{roomMessages.length}</span><p>Room messages</p></div>
          </div>
          <div className="grid two">
            <div className="card"><h3>Today’s gentle prompt</h3><p>What is one thing you wish someone understood about your day?</p><button onClick={() => setActive('journal')}>Write now</button></div>
            <div className="card"><h3>Suggested room</h3><p>2 AM Thoughts — a soft space for late-night emotions.</p><button onClick={() => setActive('rooms')}>Join room</button></div>
          </div>
        </section>}
        ...
        {active === 'journal' && <section className="page card wide"><h2>Private Journal</h2><textarea className="largeText" value={journalText} onChange={e => setJournalText(e.target.value)} placeholder="Write privately. This MVP stores entries in your browser until Supabase is connected." /><button onClick={saveJournal}>Save Journal</button><div className="list">{journals.map((j, i) => <div className="listItem" key={i}><p>{j.text}</p><small>{j.at} · Mood {j.mood}/10</small></div>)}</div></section>}
        ...
        <footer>Quiet Circle MVP · Privacy-first prototype · Connect Supabase next for cloud storage</footer>
      </main>
    </div>
  );
}
