...
const showSourceLabels = import.meta.env.DEV;
...
export default function App(){
  ...
  async function sendRoomMessage(){if(!roomInput.trim()||aiTyping)return;const userText=roomInput.trim();const msg={user:profile.name||'Anonymous',text:userText,type:'human'};const nextForRoom=[...roomMessages,msg];const nextAll={...roomMessagesById,[currentRoom.id]:nextForRoom};setRoomMessagesById(nextAll);localStorage.setItem('qc_room_messages_by_id',JSON.stringify(nextAll));setRoomInput('');if(isSupabaseConfigured){await supabase.from('room_messages').insert({room_key:currentRoom.id,user_id:session?.user?.id,display_name:msg.user,message:msg.text});setCloudStatus('Room message synced');}if(isCrisisText(userText)){setActive('crisis');await reportContent('crisis_keyword',userText);}setAiTyping(true);try{const reply=await getAgentReply(userText,currentRoom,nextForRoom);setTimeout(()=>{const buddyMsg={user:currentRoom.aiName,text:reply.text,type:'buddy',source:reply.source};const latest=getSaved('qc_room_messages_by_id',nextAll);const updated={...latest,[currentRoom.id]:[...(latest[currentRoom.id]||[]),buddyMsg]};setRoomMessagesById(updated);localStorage.setItem('qc_room_messages_by_id',JSON.stringify(updated));setAiTyping(false);},humanTypingDelay(reply.text));}catch(error){const buddyMsg={user:currentRoom.aiName,text:chatErrorText,type:'buddy',source:'error'};const latest=getSaved('qc_room_messages_by_id',nextAll);const updated={...latest,[currentRoom.id]:[...(latest[currentRoom.id]||[]),buddyMsg]};setRoomMessagesById(updated);localStorage.setItem('qc_room_messages_by_id',JSON.stringify(updated));setAiTyping(false);}}
  ...
  return <div className="appShell"><Sidebar profile={profile} session={session} nav={nav} active={active} setActive={changeActive} cloudStatus={cloudStatus} signOut={signOut} theme={theme} setTheme={setTheme}/><main className="main">
    ...
    {active==='rooms'&&<section className="page circlesPage">{!roomOpen?<><p className="eyebrow">Choose your circle</p><h2>What are you carrying today?</h2><p className="muted pageIntro">Pick a room first. The chat opens inside that circle so the experience feels focused and private.</p><div className="grid circleGrid">{rooms.map(r=><RoomCard key={r.id} room={r} selected={currentRoom.id===r.id} onSelect={openRoom}/>)}</div></>:<div className="card wide circleChatCard"><div className="chatHeader"><button className="secondaryBtn" onClick={backToRooms}>← All circles</button><div><p className="eyebrow">{currentRoom.theme}</p><h2>{currentRoom.icon} {currentRoom.name}</h2><p className="muted">Anonymous room. If the room is quiet, a Quiet Circle buddy may keep you company.</p></div></div><div className="chatWindow">{roomMessages.map((m,i)=>{const isBuddy=m.type==='buddy'||rooms.some(room=>m.user===room.aiName);return <div className={m.user===profile.name?'msg user':'msg bot'} key={`${m.user}-${i}`}><strong>{m.user}{isBuddy&&<small>{sourceLabel(m.source)}</small>}</strong><p>{m.text}</p><MessageActions message={m} isBuddy={isBuddy} onReport={text=>reportContent('message',text)}/></div>;})}{aiTyping&&<div className="typingBubble">{currentRoom.aiName} is typing…</div>}</div><div className="composer"><input value={roomInput} onChange={e=>setRoomInput(e.target.value)} placeholder={`Message ${currentRoom.name}...`} onKeyDown={e=>e.key==='Enter'&&sendRoomMessage()}/><button onClick={sendRoomMessage} disabled={aiTyping}>Send</button></div></div>}</section>}
