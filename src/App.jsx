...
export default function App() {
  ...
  async function sendCompanionMessage() {
    ...
    if (hasPersonalInfo(userText)) { const warning = { from: AI_AGENT.name, text: privacyReply(), type: 'system', id: Date.now() }; setMessages(prev => [...prev, { from: 'user', text: userText, id: Date.now() - 1 }, warning]); setChatInput(''); return; }
    if (isCrisisText(userText)) { const safeMsg = { from: AI_AGENT.name, text: safetyReply(), type: 'system', id: Date.now() }; setMessages(prev => [...prev, { from: 'user', text: userText, id: Date.now() - 1 }, safeMsg]); setChatInput(''); return; }
    ...
    try { const response = await fetch('/api/companion', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: userText, mode: companionMode.id, modePrompt: companionMode.prompt, history: messages.slice(-10) }) }); const data = await response.json().catch(() => ({})); const replyMsg = { from: AI_AGENT.name, text: (data.reply || humanReply(userText)) + sourceLabel(data.source), type: 'ai', id: Date.now() }; setMessages(prev => [...prev, replyMsg]); localStorage.setItem('qc_messages', JSON.stringify([...messages, userMsg, replyMsg])); } catch { setMessages(prev => [...prev, { from: AI_AGENT.name, text: humanReply(userText), type: 'ai', id: Date.now() }]); } finally { setCompanionTyping(false); }
  }
  async function sendRoomMessage() {
    ...
    try { const { text: agentText, source } = await getAgentReply(userText, currentRoom, nextRoomMessages); const agentMsg = { id: `agent-${Date.now()}`, user: AI_AGENT.name, senderId: AI_AGENT.id, text: agentText + sourceLabel(source), type: 'ai', source, created_at: new Date().toISOString() }; setRoomMessagesById(prev => { const updated = { ...prev, [currentRoom.id]: uniqueById([...(prev[currentRoom.id] || []), agentMsg]) }; if (!isSupabaseConfigured) localStorage.setItem('qc_room_messages_by_id', JSON.stringify(updated)); return updated; }); if (isSupabaseConfigured && session?.user?.id) await supabase.from('room_messages').insert({ room_key: currentRoom.id, user_id: session.user.id, display_name: AI_AGENT.name, message: agentText, message_type: 'ai', source }); } finally { setAiTyping(false); }
  }
