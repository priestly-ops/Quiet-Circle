...
export default function App(){
  ...
  async function signOut(){if(isSupabaseConfigured&&session)await supabase.auth.signOut();localStorage.removeItem('qc_entered');setSession(null);setEntered(false);setCloudStatus(isSupabaseConfigured?'Signed out':'Your data is saved on this device');}
  ...
  async function sendCompanionMessage(){if(!chatInput.trim())return;const userText=chatInput.trim();const userMsg={from:'You',text:userText};const next=[...messages,userMsg];setMessages(next);localStorage.setItem('qc_messages',JSON.stringify(next));setChatInput('');if(isCrisisText(userText)){const safe=[...next,{from:'Quiet Circle',text:safetyReply()}];setMessages(safe);localStorage.setItem('qc_messages',JSON.stringify(safe));setActive('crisis');return;}const reply=await getAgentReply(userText,{name:'Private companion',theme:companionMode.label,id:companionMode.id},next.map(m=>({user:m.from,text:m.text})));const updated=[...next,{from:'Quiet Circle',text:reply}];setMessages(updated);localStorage.setItem('qc_messages',JSON.stringify(updated));}

  async function sendRoomMessage(){if(!roomInput.trim())return;const userText=roomInput.trim();const msg={user:profile.name||'Anonymous',text:userText,type:'human'};const nextForRoom=[...roomMessages,msg];const nextAll={...roomMessagesById,[currentRoom.id]:nextForRoom};setRoomMessagesById(nextAll);localStorage.setItem('qc_room_messages_by_id',JSON.stringify(nextAll));setRoomInput('');if(isSupabaseConfigured){await supabase.from('room_messages').insert({room_key:currentRoom.id,user_id:session?.user?.id,display_name:msg.user,message:msg.text});setCloudStatus('Room message synced');}if(isCrisisText(userText)){setActive('crisis');await reportContent('crisis_keyword',userText);}setAiTyping(true);const reply=await getAgentReply(userText,currentRoom,nextForRoom);const buddyMsg={user:currentRoom.aiName,text:reply,type:'buddy'};const latest=getSaved('qc_room_messages_by_id',nextAll);const updated={...latest,[currentRoom.id]:[...(latest[currentRoom.id]||[]),buddyMsg]};setRoomMessagesById(updated);localStorage.setItem('qc_room_messages_by_id',JSON.stringify(updated));setAiTyping(false);}
