...
export default function App(){
  ...
  async function enterApp(mode){const next={...profile,name:profile.name||makeAnonName()};setProfile(next);localStorage.setItem('qc_profile',JSON.stringify(next));if(!isSupabaseConfigured||mode==='guest'){localStorage.setItem('qc_entered',JSON.stringify(true));setEntered(true);setAuthMode(mode);navigate('/dashboard',{replace:true});return;}try{if(mode==='google'){localStorage.setItem('qc_profile',JSON.stringify(next));await supabase.auth.signInWithOAuth({provider:'google',options:{redirectTo:window.location.origin}});return;}if(!email.trim()){setAuthNotice('Enter your email first.');return;}const{error}=await supabase.auth.signInWithOtp({email:email.trim(),options:{emailRedirectTo:window.location.origin}});if(error)throw error;await syncProfile(next,null,'email_magic_link');setAuthNotice('Magic link sent. Check your email. For beta preview, you can continue as guest.');}catch(err){setAuthNotice(err.message||'Auth setup needs provider configuration. Continuing in beta mode.');}}
  ...
  async function sendCompanionMessage(){if(!chatInput.trim()||companionTyping)return;const userText=chatInput.trim();const userMsg={from:'You',text:userText};const next=[...messages,userMsg];setMessages(next);localStorage.setItem('qc_messages',JSON.stringify(next));setChatInput('');if(isCrisisText(userText)){const safe=[...next,{from:'Quiet Circle',text:safetyReply()}];setMessages(safe);localStorage.setItem('qc_messages',JSON.stringify(safe));changeActive('crisis');return;}setCompanionTyping(true);const reply=await getAgentReply(userText,{name:'Private companion',theme:companionMode.label,id:companionMode.id},next.map(m=>({user:m.from,text:m.text})));setTimeout(()=>{const updated=[...next,{from:'Quiet Circle',text:reply}];setMessages(updated);localStorage.setItem('qc_messages',JSON.stringify(updated));setCompanionTyping(false);},humanTypingDelay(reply));}
